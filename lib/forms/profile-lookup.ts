...
export function formHasProfileLookup(form: Pick<ChurchForm, 'category' | 'enable_profile_lookup'>): boolean {
  ...
  if (form.category === CAMP_MEETING_FEEDBACK_CATEGORY) return true
  return Boolean(form.enable_profile_lookup)
}

export function isCampMeetingFeedbackForm(form: Pick<ChurchForm, 'category'>): boolean {
  return form.category === CAMP_MEETING_FEEDBACK_CATEGORY
}
